...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Training/DAO/" ] && mkdir -p "$EXTROOT/CRM/Training/DAO/"
  cp -f "$CIVIROOT/CRM/Training/DAO"/* "$EXTROOT/CRM/Training/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Training" "$EXTROOT/CRM/Training/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
